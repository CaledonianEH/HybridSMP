...
public class Ugly implements CommandHandler {
    @Override
    public void execute(CommandSender sender, Command command, String[] args) {
        if(sender instanceof Player){
            ...
            Bukkit.broadcastMessage(Utils.chat(Files.msgs.getString("reactions.ugly").replace("%prefix%", prefix).replace("%player%", player)));
