...
public class Survival implements CommandHandler {
    @Override
    public void execute(CommandSender sender, Command command, String[] args) {
        String prefix = Files.msgs.getString("prefix");
        String gamemode = Files.msgs.getString("gamemodes.names.survival");
        if(sender instanceof Player){
            Player p = (Player) sender;
            if(args.length == 0){
                // Player
                if(p.hasPermission(Files.perms.getString("survival.self"))){
                ...
                if(p.hasPermission(Files.perms.getString("survival.other"))){
